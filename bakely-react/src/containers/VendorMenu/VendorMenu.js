...
import * as classModules from './VendorMenu.css';
  ...
  render() {
      ...
      if (this.props.userId === item.vendorId) {
        itemList.push(
          <MenuItemPreview
            ...
            author={author}
            onClick={() => {
              this.addEditAnItem(item, "editing");
            }}
          />
        );
      }
    ...
    return (
      <Grid container spacing={1} justify={"center"} alignContent={"center"}>
        <Modal
          show={this.state.addingEditingAnItem}
        ...
        >
          <VendorAddEditMenuItem
            ...
            onAddEditMenuItem={(formData) =>
              this.handleAddEditMenuItem(formData)
            }
            onDeleteMenuItem={(foodId) => this.handleDeleteMenuItem(foodId)}
          />
        </Modal>
        <Grid item xs={12}>
          <div className={classModules.title}>
            My Menu
          </div>
        </Grid>
        <Grid item xs={12}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    onAddEditFoodItem: (formData) =>
      dispatch(actions.addEditFoodItem(formData)),
